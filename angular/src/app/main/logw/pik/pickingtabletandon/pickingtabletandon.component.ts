import { CommonFunction } from './../../../commonfuncton.component';
...
import { MstLgwPickingTabletProcessServiceProxy,
             MstLgwPickingTabletProcessGetdataOutput, } from '@shared/service-proxies/service-proxies';



@Component({
  selector: 'app-pickingtablet',
  templateUrl: './pickingtabletandon.component.html',
  styleUrls: ['./pickingtabletandon.component.less']
})


export class PickingTabletAndonComponent extends AppComponentBase {

    @ViewChild('popupUnpack', { static: true }) popupUnpack:| PopupUnpackComponent| undefined;
    rowData: MstLgwPickingTabletProcessGetdataOutput[] = []
    rowCount:number;
    row1:number;
    ...
    loadtotal:number;
        ...
        drawScreen(){
            ...
            let cleft = document.querySelectorAll<HTMLElement>('.PICKINGTABLET .contentf .contleft .itemp').length;
            let cright = document.querySelectorAll<HTMLElement>('.PICKINGTABLET .contentf .contright .itemp').length;
            let itemcount = ((cleft > cright) ? cleft: cright) + 1;

            let hitem = Math.floor(((hcont + 1) / itemcount) * 100) / 100;
            let hitem2 = Math.floor((hitem / 2) * 100) / 100;
            ...
            for(let i=0; _item2[i]; i++){
                _item2[i].style.height = hitem2 - 5 + 'px';
                _item2[i].style.lineHeight = hitem2 - 5 + 'px'; }

            let _muiten = document.querySelectorAll<HTMLElement>('.PICKINGTABLET .cont .icol hr');
            for(let i=0; _muiten[i]; i++){ _muiten[i].style.marginTop = hitem4 - 1 + 'px'; }
            let _muiten2 = document.querySelectorAll<HTMLElement>('.PICKINGTABLET .cont .icol img');
            for(let i=0; _muiten2[i]; i++){ _muiten2[i].style.marginTop = hitem4 - 11 + 'px'; }
            let _muiten3 = document.querySelector<HTMLElement>('.PICKINGTABLET .cont .muitenlen');
            _muiten3.style.height = (hitem2 - 10) + 'px';
            _muiten3.style.top =  '-'+ (hitem2 - 5) + 'px';
            _muiten3.style.right =  '-'+ ((hitem2 - 10) /2) + 'px';

            let hbox3val = document.querySelectorAll<HTMLElement>('.PICKINGTABLET .cont .box3 .val');
            for(let i=0; hbox3val[i]; i++){
                hbox3val[i].style.height = hitem3*2 - 2 + 'px';
                hbox3val[i].style.lineHeight = hitem3*2 - 2 + 'px'; }
            let hbox3tit = document.querySelectorAll<HTMLElement>('.PICKINGTABLET .cont .box3 .tit');
            for(let i=0; hbox3tit[i]; i++){
                hbox3tit[i].style.height = hitem3 - 2 + 'px';
                hbox3tit[i].style.lineHeight = hitem3 - 2 + 'px';}
        ...
        initLayoutData(){
            ...
            this. _serviceProxy.mstLgwPickingTabletProcessGetdata(this.PickingPosition, this.TabletId)
                .pipe(finalize(() => { this.completeLayoutData(countload) }))
                .subscribe((result) => {
                    if(result){
                        this.rowCount = result.length;
                        this.row1 = Math.floor((this.rowCount / 2));
        ...
        BIND_DATA(_data:MstLgwPickingTabletProcessGetdataOutput[]){
            _data.forEach(d => {
                let el = document.querySelector<HTMLElement>('.PICKINGTABLET .contentf .cont .itemp.index' + d.logicSequenceNo);
                if(el){
                    ...
                    ispushedBox3.classList.add(_isPushed);
                    ...
                    _process.textContent = d.process;
        ...
        UNPACK_PXP() {
            this.popupUnpack.show(this.PickingPosition, this.TabletId);
        }
        ...
        dataleft_bk(){
            let numRange: MstLgwPickingTabletProcessGetdataOutput[] = [];
            for (let i = 1; i <= this.row1;) {
                numRange.push(this.rowData[i-1]);
                i = i + 1;
            }
            return numRange;
        }
        dataright_bk(){
            let numRangeDesc: MstLgwPickingTabletProcessGetdataOutput[] = [];
            for (let i = this.rowCount; i > this.row1;) {
                numRangeDesc.push(this.rowData[i-1]);
                i = i -1;
            }
            return numRangeDesc;
        }
        ...
        dataleft(){
            let numRangeDesc: MstLgwPickingTabletProcessGetdataOutput[] = [];
            if(this.row1){
                for (let i = this.row1; i < this.rowCount; i++) {
                    numRangeDesc.push(this.rowData[i]);
                }
            }
            return numRangeDesc;
        }
        dataright(){
            let numRange: MstLgwPickingTabletProcessGetdataOutput[] = [];
            for (let i = this.row1 -1 ; i >= 0; i--) {
                numRange.push(this.rowData[i]);
            }
            return numRange;
        }
        ...
        isNull(_active: any, _default: string){
