...
import { LgwPupPxPUpPlanServiceProxy } from '@shared/service-proxies/service-proxies';
...
@Component({
  selector: 'import-pxpupplan-modal',
  templateUrl: './import-pxpupplan-modal.component.html',
  styleUrls: ['./import-pxpupplan-modal.component.less']
})
export class ImportPxpUpPlanComponent   extends AppComponentBase {
    ...
    constructor(
      ...
      private _pxpUpPlan: PxPUpPlanComponent
    ) {
      super(injector);
      this.uploadUrl = AppConsts.remoteServiceBaseUrl + '/Prod/ImportPxPUpPlanFromExcel';
      {}
    }
    ngOnInit() {
    ...
    upload() {
        ...
        this._httpClient
            ...
            .subscribe(response => {
                if (response.success ) {
                    this.mergePxpUpPlan(response.result.framPlan.result[0].guid);
    ...
    mergePxpUpPlan(guid) {
        this.isLoading = true;
        this._service.mergeDataPxPUpPlan(guid)
            .pipe(
                finalize(() => {
                    this.isLoading = false;
                })
