...
export class TmssDatepickerComponent extends AppComponentBase implements ControlValueAccessor, OnInit {
	...
	@Input() DisabledTxt: string = 'false';
	...
	writeValue(val: any) {
		setTimeout(()=>{
            ...
            const dateValue = new Date(Date.parse(moment(val.toString() ?? this.defaultValue)?.format('YYYY-MM-DDTHH:mm:ss')));
