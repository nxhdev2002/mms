...
import { MstLgaBp2PartListServiceProxy } from '@shared/service-proxies/service-proxies';
...
@Component({
  selector: 'import-bp2partlist-modal',
  templateUrl: './import-bp2partlist-modal.component.html',
  styleUrls: ['./import-bp2partlistt-modal.component.less']
})
export class ImportBp2PartListComponent   extends AppComponentBase {
    ...
    constructor(
      ...
      private _partlist: Bp2PartListComponent
    ) {
      super(injector);
      this.uploadUrl = AppConsts.remoteServiceBaseUrl + '/Prod/ImportBp2PartListFromExcel';
      {}
    }
    ngOnInit() {
    ...
    upload() {
            if(this.fileName != ''){
                ...
                .subscribe(response => {
                    ...
                    if (response.success ) {
                         this.mergeDataBp2PartList(response.result.mstLgaBp2PartList.result[0].guid);
                    }
                    else if (response.error != null || !response.result.mstLgaBp2PartList.result) {
    ...
    mergeDataBp2PartList(guid) {
        this.isLoading = true;
        this._service.mergeDataBp2PartList(guid)
            .pipe(
                finalize(() => {
                    this.isLoading = false;
                })
