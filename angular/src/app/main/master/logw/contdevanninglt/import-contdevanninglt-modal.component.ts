...
import { MstLgwContDevanningLTServiceProxy } from '@shared/service-proxies/service-proxies';
...
@Component({
  selector: 'import-contdevanninglt-modal',
  templateUrl: './import-contdevanninglt-modal.component.html',
  styleUrls: ['./import-contdevanninglt-modal.component.less']
})
export class ImportContDevanningLTComponent   extends AppComponentBase {
    ...
    constructor(
      ...
      private _contdevanninlts: ContDevanningLTComponent
    ) {
      super(injector);
      this.uploadUrl = AppConsts.remoteServiceBaseUrl + '/Prod/ImportContDevanningLTFromExcel';
      {}
    }
    ngOnInit() {
    ...
    upload() {
            if(this.fileName != ''){
                ...
                .subscribe(response => {
                    if (response.success ) {
                        this.mergeDataContDevanningLT(response.result.unpackingPart.result[0].guid);
    ...
    mergeDataContDevanningLT(guid) {
        this.isLoading = true;
        this._service.mergeDataContDevanningLT(guid)
            .pipe(
                finalize(() => {
                    this.isLoading = false;
                })
            ...
            .subscribe(() => {
                ...
                this.close();
                this._contdevanninlts.searchDatas();
            });
    }
