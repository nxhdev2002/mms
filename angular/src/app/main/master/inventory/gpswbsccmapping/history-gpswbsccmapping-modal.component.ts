...
import { MstGpsWbsCCMappingServiceProxy } from "@shared/service-proxies/service-proxies";
...
@Component({
    selector: 'history-gpswbsccmapping-modal',
    templateUrl: './history-gpswbsccmapping-modal.component.html'
})
export class ViewHistoryGpsWbsCCMappingModalComponent extends AppComponentBase {
    @ViewChild('viewHistoryGpsWbsCCMappinglModal', { static: true }) modal: ModalDirective | undefined
    ...
    show(id?): void {
        if(id) {
            ...
            this.isLoading = true
            this.allRowHeaders = [];
            this.getData(id,'MstGpsWbsCCMapping')
        } 
    }
    getData(_id,_tableName){
        this._service.getGpsWbsCCMappingHistory(
            _id,
            _tableName,
            '',
