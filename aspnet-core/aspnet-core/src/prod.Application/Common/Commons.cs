...
namespace prod.Common
{
    public class Commons
    {
        ...
        public static void FillHistoriesExcel(List<JObject> data, ExcelWorksheet v_worksheet, int startRow, int startCol, string[] properties, string[] p_header)
        {
            DataTable table = ConvertJsonToDataTable(data, p_header);
        ...
        public static DataTable ConvertJsonToDataTable(List<JObject> rowDatas, string[] headers)
        {
            DataTable dataTable = new DataTable();

            // Assuming all JSON strings have the same structure
            foreach (var header in headers.ToArray())
            {
                dataTable.Columns.Add(header, typeof(string));
            }

            foreach (JObject row in rowDatas)
            {
                //JObject jsonObject = JObject.Parse(row);
                DataRow rowTable = dataTable.NewRow();
                foreach (JProperty header in row.Properties())
                {
                    if (headers.Contains(header.Name))
                        rowTable[header.Name] = header.Value.ToString() ?? "";
                }
        ...
        public static string getConnectionString(string property = "Default")
        {
            ...
            return connectionStrings.Property(property).Value.ToString();
        }
