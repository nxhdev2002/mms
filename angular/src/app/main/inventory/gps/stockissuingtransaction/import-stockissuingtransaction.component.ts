...
import { InvGpsStockIssuingTransactionServiceProxy } from '@shared/service-proxies/service-proxies';
...
@Component({
  selector: 'import-stockissuingtransaction',
  templateUrl: './import-stockissuingtransaction.component.html',
  styleUrls: ['../../../import-modal.less'],
})
export class ImportGpsStockIssuingTransactionComponent extends AppComponentBase {
    @ViewChild('imgInput', { static: false }) InputVar: ElementRef;
    @ViewChild('ExcelFileUpload', { static: false }) excelFileUpload: FileUpload;
    @ViewChild('importModal', { static: true }) modal: ModalDirective;
    @ViewChild('errorListModal', { static: true }) errorListModal: ListErrorImportGpsStockIssuingTransactionComponent;
    ...
    constructor(
        ...
        private _stockIssuing: StockIssuingTransactionComponent
    ) {
        super(injector);
        this.uploadUrl = AppConsts.remoteServiceBaseUrl + '/Prod/ImportGpsStockIssuingTransactionFromExcel';
        { }
    }
    ngOnInit() {
    }
    ...
    upload() {
        ...
        if (this.fileName != '') {
            ...
            this._httpClient
                ...
                .subscribe(response => {
                    if (response.success) {
                        if (response.result.gpsIssuingTrans.result.length == 0) {
                            this.exeptionDataImport();
                        }
                        else {
                            this.mergeDrmStockPart(response.result.gpsIssuingTrans.result[0].guid);
                        }
                    }
                },
                (error) => {
    ...
    mergeDrmStockPart(guid) {
        this.isLoading = true;
        this._service.mergeDataInvGpsStockIssuing(guid)
            .pipe(
                finalize(() => {
        ...
        this._service.getMessageErrorImport(guid)
            .subscribe((result) => {
                ...
                if (result.items.length > 0) {
                    ...
                    this.modal.hide();
                    this.modalClose.emit(null);
                    this._stockIssuing.searchDatas();
                    this.close();
                }
            });
    }
