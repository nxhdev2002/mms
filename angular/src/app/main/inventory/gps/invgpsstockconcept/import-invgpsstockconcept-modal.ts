...
import { InvGpsStockConceptServiceProxy } from '@shared/service-proxies/service-proxies';
...
@Component({
  ...
  selector: 'import-invgpsstockconcept-modal',
  templateUrl: './import-invgpsstockconcept-modal.component.html',
  styleUrls: ['../../../import-modal.less'],
})
export class ImportInvGpsStockConceptComponent   extends AppComponentBase {
    ...
    constructor(
      ...
      private _contRental: InvGpsStockConceptComponent
    ) {
      super(injector);
      this.uploadUrl = AppConsts.remoteServiceBaseUrl + '/Prod/ImportInvGpsStockConceptFromExcel';
      {}
    }
    ngOnInit() {
    ...
    upload() {
        ...
        if (this.fileName != '') {
            ...
            this._httpClient
                ...
                .subscribe(response => {
                    if (response.success) {
                        if (response.result.stockConcept.result.length == 0) {
    ...
    mergeStockConcept(guid) {
        this.isLoading = true;
        this._service.mergeDataInvGpsStockConcept(guid)
            .pipe(
                finalize(() => {
