...
@Component({
  ...
  templateUrl: './import-drmstockpart.component.html',
  styleUrls: ['../../../import-modal.less'],
})
export class ImportDrmStockPartComponent extends AppComponentBase {
    @ViewChild('imgInput', { static: false }) InputVar: ElementRef;
    @ViewChild('ExcelFileUpload', { static: false }) excelFileUpload: FileUpload;
    @ViewChild('importModal', { static: true }) modal: ModalDirective;
    @ViewChild('errorListModal', { static: true }) errorListModal: ListErrorImportDrmStockPartComponent;
    ...
    constructor(
        ...
        private _drmStockPart: DrmStockPartComponent
    ) {
        super(injector);
        this.uploadUrl = AppConsts.remoteServiceBaseUrl + '/Prod/ImportDrmStockPartFromExcel';
        { }
    }
    ngOnInit() {
    }
    ...
    upload() {
        ...
        if (this.fileName != '') {
            ...
            this._httpClient
                ...
                .subscribe(response => {
                    if (response.success) {
                        if (response.result.drmstockpart.result.length == 0) {
                            this.exeptionDataImport();
                        }
                        else {
                            this.mergeDrmStockPart(response.result.drmstockpart.result[0].guid);
                        }
                    }
                },
                (error) => {
