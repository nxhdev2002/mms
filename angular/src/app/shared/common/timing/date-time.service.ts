...
export class DateTimeService {
    ...
    convertToDatetime(d:Date) {
        if(d) return DateTime.local(d.getFullYear(), d.getMonth() + 1, d.getDate(), d.getHours(), d.getMinutes(), d.getSeconds(),  d.getMilliseconds(), { zone: 'Asia/Ho_Chi_Minh' });
        else undefined;
    }
    convertToDate(d:Date) {
        if(d) return DateTime.local(d.getFullYear(), d.getMonth() + 1, d.getDate() );
        else undefined;
    }
    getTime(d:any) {
        if(d == null) return "";
        ...
        console.log(d)
        if(d) return d.getHours()+":"+d.getMinutes()+":"+d.getSeconds();
        // if (d) d.toString();
        else "";
    }
}
