...
namespace prod.Inventory.CKD.ProductionPlanMonthly
{
     [AbpAuthorize(AppPermissions.Pages_ProdPlan_ProductionPlanMonthly_View)]
    public class InvCkdProductionPlanMonthlyAppService : prodAppServiceBase, IInvCkdProductionPlanMonthlyAppService
    {

        private readonly IDapperRepository<InvCkdProductionPlanMonthly, long> _dapperRepo;
        private readonly IInvCkdProductionPlanMonthlyExcelExporter _calendarListExcelExporter;

        public InvCkdProductionPlanMonthlyAppService(IDapperRepository<InvCkdProductionPlanMonthly, long> dapperRepo,
                                         IInvCkdProductionPlanMonthlyExcelExporter calendarListExcelExporter
            )
        {
            _dapperRepo = dapperRepo;
            _calendarListExcelExporter = calendarListExcelExporter;
        }
        public async Task<PagedResultDto<InvCkdProductionPlanMonthlyDto>> GetAllProductionPlanMonthly(InvCkdProductionPlanMonthlyInput input)
        {
            string _ProdMonth = (input.ProdMonth.ToString("dd-MM-yyyy") != "01-01-0001") ? input.ProdMonth.ToString("yyyy-MM") : "";
            string _sql = "Exec INV_CKD_PRODUCTION_PLAN_MONTHLY_SEARCH @p_cfc, @p_grade, @prodMonth";

            IEnumerable<InvCkdProductionPlanMonthlyDto> result = await _dapperRepo.QueryAsync<InvCkdProductionPlanMonthlyDto>(_sql, new
            {
                p_cfc = input.Cfc,
                p_grade = input.Grade,
            ...
            return new PagedResultDto<InvCkdProductionPlanMonthlyDto>(
                totalCount,
                pagedAndFiltered);
        }

        public async Task<FileDto> GetInvCkdProdPlanMonthlyToExcel(InvCkdProductionPlanMonthlyExportInput input)
        {
            string _ProdMonth = (input.ProdMonth.ToString("dd-MM-yyyy") != "01-01-0001") ? input.ProdMonth.ToString("yyyy-MM") : "";
            string _sql = "Exec INV_CKD_PRODUCTION_PLAN_MONTHLY_SEARCH @p_cfc, @p_grade, @prodMonth";
        ...
        [AbpAuthorize(AppPermissions.Pages_ProdPlan_ProductionPlanMonthly_Import)]
        public async Task<List<InvCkdProductionPlanMonthlyImportDto>> ImportProductionPlanMonthlyFromExcel(byte[] fileBytes, string fileName)
        {
            try
            {
                List<InvCkdProductionPlanMonthlyImportDto> listImport = new List<InvCkdProductionPlanMonthlyImportDto>();
                using (var stream = new MemoryStream(fileBytes))
                {
                    ...
                    for (int i = startrow; i < v_worksheet.Rows.Count; i++)
                    {
                        ...
                        if (v_grade != "")
                        {
                            ...
                            var row = new InvCkdProductionPlanMonthlyImportDto();
                                row.Guid = strGUID;
                                row.Cfc = v_model;
                                row.Grade = v_grade;
                    ...
                    {
                        IEnumerable<InvCkdProductionPlanMonthlyImportDto> dataE = listImport.AsEnumerable();
                        ...
                        using (Microsoft.Data.SqlClient.SqlConnection conn = new Microsoft.Data.SqlClient.SqlConnection(connectionString))
                        {
                            ...
                            using (Microsoft.Data.SqlClient.SqlTransaction tran = conn.BeginTransaction(IsolationLevel.ReadCommitted))
                            {
                                ...
                                {
                                    bulkCopy.DestinationTableName = "InvCkdProductionPlanMonthly_T";
        ...
        //Merge Data 
        public async Task MergeDataInvCkdProdPlanMonthly(string v_Guid)
        {
            string _sql = "Exec INV_CKD_PRODUCTION_PLAN_MONTHLY_MERGE @Guid";
            await _dapperRepo.QueryAsync<InvCkdProductionPlanMonthlyImportDto>(_sql, new { Guid = v_Guid });
        }

        // hiển thị lỗi import và export lỗi
        public async Task<PagedResultDto<InvCkdProductionPlanMonthlyImportDto>> GetMessageErrorImport(string v_Guid)
        {
            string _sql = "Exec INV_CKD_PRODUCTION_PLAN_MONTHLY_GET_LIST_ERROR_IMPORT @Guid";

            IEnumerable<InvCkdProductionPlanMonthlyImportDto> result = await _dapperRepo.QueryAsync<InvCkdProductionPlanMonthlyImportDto>(_sql, new
            {
                Guid = v_Guid
            });
            ...
            return new PagedResultDto<InvCkdProductionPlanMonthlyImportDto>(
                totalCount,
               listResult
               );
        }
