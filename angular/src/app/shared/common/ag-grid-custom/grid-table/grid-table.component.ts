...
import { ClipboardModule, MenuModule, MultiFilterModule, RangeSelectionModule, RichSelectModule, RowGroupingModule, SetFilterModule } from '@ag-grid-enterprise/all-modules';
...
export class GridTableComponent extends AppComponentBase implements OnInit {
    ...
    @Input() rowData: any[] = [];
    @Input() modules: Module[] = [ClientSideRowModelModule, RangeSelectionModule, RowGroupingModule, RichSelectModule, SetFilterModule, MultiFilterModule, ClipboardModule, MenuModule];
    ...
    @Input() typeDropdown: boolean = false;
    ...
    @Input() showOpenedGroup: boolean = false; // show ra noi dung cua rowGroup
    ...
    ngAfterViewInit() {
        ...
        if(this.typeDropdown || !this.heightAuto) this.setHeight(this.height);  // type dropdownlist
        else this.fn.setHeight_notFullHeight(this.heightAuto,0);
    ...
    onCellKeyPress(params: AgCellEditorParams) {
        if (['Enter'].indexOf(params.event.key.toString()) !== -1) {
    ...
    onCellEditingStarted(params: AgCellEditorParams) {
        ...
        //cellEditingStarted
        // return this.cellEditingStarted.emit(params);
    }
