...
import { LgaLdsLotPlanDto, LgaLdsLotPlanServiceProxy } from '@shared/service-proxies/service-proxies';
...
@Component({
    templateUrl: './lotplan.component.html',
    styleUrls: ['./lotplan.component.less'],
})
export class LotplanComponent extends AppComponentBase implements OnInit {
    @ViewChild('createOrEditModalLotPlan', { static: true }) createOrEditModalLotPlan:| CreateOrEditLotplanModalComponent| undefined;
    ...
    selectedRow: LgaLdsLotPlanDto = new LgaLdsLotPlanDto();
    saveSelectedRow: LgaLdsLotPlanDto = new LgaLdsLotPlanDto();
    datas: LgaLdsLotPlanDto = new LgaLdsLotPlanDto();
    ...
    constructor(
        injector: Injector,
        private _service: LgaLdsLotPlanServiceProxy,
        ...
        this.defaultColDefs = [
			...
			{
                headerName: this.l('Plan Start Datetime'),
                headerTooltip: this.l('Plan Start Datetime'),
                field: 'planStartDatetime',
                valueGetter: (params) => this.pipe.transform(params.data?.planStartDatetime, 'dd/MM/yyyy HH:mm:ss'),
                width: 100,
            },
            {
            ...
            {
                headerName: this.l('Lot No 2'),
                headerTooltip: this.l('Lot No 2'),
                field: 'lotNo2',
                width: 100,
            },
			{
                headerName: this.l('Trip'),
                headerTooltip: this.l('Trip'),
                field: 'trip',
                width: 100,
                type: 'rightAligned',
            },
			{
                headerName: this.l('Dolly'),
                headerTooltip: this.l('Dolly'),
                field: 'dolly',
                width: 100,
            },
			{
    ...
    onChangeRowSelection(params: { api: { getSelectedRows: () => LgaLdsLotPlanDto[] } }) {
        this.saveSelectedRow = params.api.getSelectedRows()[0] ?? new LgaLdsLotPlanDto();
    ...
    deleteRow(system: LgaLdsLotPlanDto): void {
    ...
    exportToExcel(): void {
        this.isLoading = true
        this._service.getLotPlanToExcel(
            this._dateTimeService.convertToDatetime(this.workingDateFrom),
            this._dateTimeService.convertToDatetime(this.workingDateTo),
            this.prodLine,
            this._dateTimeService.convertToDatetime(this.workingDate)
