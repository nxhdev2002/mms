...
export class DataFormatService {
  ...
  // datetimeFormat(val: string | moment.Moment | Date | DateTime, _format: string) {
  //   return val ? moment(val).tz(this.getTimeZone()).local().format(_format) : '';
  // }

  // Ngày
  dateFormat(val: string | moment.Moment | Date) {
    // if (val == '0001-01-01T00:00:00' || val == '9999-12-31T23:59:59.9999999') return '';
    return val ? moment(val.toString())?.format('DD/MM/YYYY') : '';
  ...
  formatDisplayValue(val: any, hideDecimal?: boolean) {
    ...
    formatMoney_decimal0(val: any, _decimalMax?:number, _decimalFull?:boolean, _decimalMath?:boolean, _mathType?: string): any {
    ...
    decimal_math(_valule: number, _decimalMax?: number, _type?:string){

        let _numVal = _valule;
        let _val = _valule.toString().split('.');
        if(_val.length > 1){

            let _decimaltmp = 1;
            this.fornumbers(_decimalMax).forEach( function(value) {_decimaltmp = _decimaltmp * value; });

            let _valTrim = parseFloat(_val[1]);
            if(_type == "round") _valTrim = Math.round(parseFloat(_val[1]) * _decimaltmp) / _decimaltmp;
            else if(_type == "ceil") _valTrim = Math.ceil(parseFloat(_val[1]) * _decimaltmp) / _decimaltmp;
            else if(_type == "floor") _valTrim = Math.floor(parseFloat(_val[1]) * _decimaltmp) / _decimaltmp;

            _numVal = parseFloat(_val[0] + "." + _valTrim);
        }
  ...
  formatMoney_decimal_bk(val: any, hideDecimal?: boolean, _decimal?:number) {
    val = hideDecimal ? val : this.numberFormat(val);
    if (val) {
            if (typeof val === 'string') {
                let num = val.trim().replace(/,([0-9]{3})/g, '$1');
                ...
                if (_decimal) {
                        let _nums = val.toString().split(".");

                        let _num = _nums[0].toString().replace(/,/g, '');

                        // console.log(_num + " - " + (+_num));

                        let _num0 = (+_num) ? (+_num).toLocaleString("en-US") : _nums[0];

                        let _numVal = _num0;
                        if(_nums.length > 1) {
                            _decimal = _decimal > _nums[1].length ? _nums[1].length: _decimal;
                            _numVal = _num0 + "." + _nums[1].substr(0, _decimal);
                        }
                        ...
                        const num = val.toString().replace(/,/g, '');
                        return (+num) ? (+num).toLocaleString("en-US") : val;
                }
            }
    }
    return '';
