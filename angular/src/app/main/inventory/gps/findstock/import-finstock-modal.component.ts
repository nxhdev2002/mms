...
import { InvGpsStockConceptServiceProxy, InvGpsIssuingServiceProxy, InvGpsIssuingsServiceProxy, GetIssuingImportView, InvGpsFinStockServiceProxy } from '@shared/service-proxies/service-proxies';
...
@Component({
  ...
  selector: 'import-finstock-modal',
  templateUrl: './import-finstock-modal.component.html',
 styleUrls: ['./import-finstock-modal.component.less'],
  //styleUrls: ['./import-issuings-modal.component.less']
})
export class ImportInvGpsFinstockComponent   extends AppComponentBase {
    @ViewChild('imgInput', { static: false }) InputVar: ElementRef;
    @ViewChild('ExcelFileUpload', { static: false }) excelFileUpload: FileUpload;
    @ViewChild('importNewModal', { static: true }) modal: ModalDirective;
    @Output() modalSave: EventEmitter<any> = new EventEmitter<any>();
    @Output() modalClose: EventEmitter<any> = new EventEmitter<any>();
   // @ViewChild('errorListModal', { static: true }) errorListModal: ListErrorImportGpsFinStockComponent;
    @ViewChild('errorListModal', { static: true }) errorListModal: ErrorImportFinstockModalComponent;
    ...
    constructor(
      ...
      private _servicefinstock: InvGpsFindStockComponent
    ) {
      super(injector);
      this.uploadUrl = AppConsts.remoteServiceBaseUrl + '/Prod/ImportGpsFinStockExcel';
      {}
    }
    ngOnInit() {
    ...
    upload() {
        ...
        if (this.fileName != '') {
            ...
            this._httpClient
                ...
                .subscribe(response => {
                    if (response.success) {
                        if (response.result.finstock.length == 0) {
                            this.exeptionDataImport();
                        }
                        else {
                            this.mergeStockConcept(response.result.finstock[0].guid);
                        }
                    }
                },
                (error) => {
    ...
    mergeStockConcept(guid) {
        ...
        this.isLoading = true;
        this._service.mergeDataInvGpsFinStock(guid)
            .pipe(
                finalize(() => {
        ...
        this._service.getMessageErrorImport(guid)
            .subscribe((result) => {
                ...
                if (result.items.length > 0) {
                    ...
                    this.modal.hide();
                    this.modalClose.emit(null);
                    this._servicefinstock.searchDatas();
                    this.close();
                }
            });
    }
