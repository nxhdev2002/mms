...
import { MstLgwUnpackingPartServiceProxy } from '@shared/service-proxies/service-proxies';
...
@Component({
  selector: 'import-unpackingpart-modal',
  templateUrl: './import-unpackingpart-modal.component.html',
  styleUrls: ['./import-unpackingpart-modal.component.less']
})
export class ImportUnpackingPartComponent   extends AppComponentBase {
    ...
    constructor(
      ...
      private _unpackingPart: UnpackingPartComponent
    ) {
      super(injector);
      this.uploadUrl = AppConsts.remoteServiceBaseUrl + '/Prod/ImportUnpackingPartFromExcel';
      {}
    }
    ngOnInit() {
    ...
    mergeUnpackingPart(guid) {
        this.isLoading = true;
        this._service.mergeDataUnpackingPart(guid)
            .pipe(
                finalize(() => {
                    this.isLoading = false;
                })
