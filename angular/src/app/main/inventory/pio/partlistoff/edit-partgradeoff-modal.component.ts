...
import { GetGradebyPartListDto, InvCkdPartGradeDto, InvCkdPartListServiceProxy, InvPartListOffServiceProxy, InvPioPartGradeInlDto, InvPioPartListInlServiceProxy } from '@shared/service-proxies/service-proxies';
...
@Component({
    selector: 'edit-partgradeoff-modal',
    templateUrl: './edit-partgradeoff-modal.component.html',
    styleUrls: ['./edit-partgradeoff-modal.component.less']
})
export class EditPartGradeOffModalComponent extends AppComponentBase {
    @ViewChild('editPartGradeOffModal', { static: true }) modal: ModalDirective | undefined;
    ...
    rowdata: InvPioPartGradeInlDto = new InvPioPartGradeInlDto();
    rowdatagrade = [];
    ...
    show(rowdata?: any, rowDataPartGrade?: any): void {
        this.bodyColorList = [];
        this.gradeListc = [];
        this.colorList = [];
        this.gradeLists1 = [];
        ...
        this.rowdatagrade = rowDataPartGrade;
    ...
    save(): void {
        ...
        else if(this.bodyColorList.length > 0){
            this.bodyColorFrist = this.bodyColorList[0] == null ? this.bodyColorList[1] : this.bodyColorList[0];
            for(var  i = 0;i < this.rowdatagrade.length; i++){
                if(this.rowdatagrade[i].grade == this._grade && this.rowdatagrade[i].bodyColor == this.bodyColorFrist){
                    this.message.warn(this.l('Color này đã tồn tại rồi!'));
                    this.checkEdit = false;
                    return;
                }
            }
        ...
        if (this.checkEdit) {
            ...
            this._service.partGradeOffEdit(input)
