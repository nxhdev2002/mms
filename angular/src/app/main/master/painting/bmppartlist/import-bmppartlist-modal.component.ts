...
import { MstPtsBmpPartListServiceProxy } from '@shared/service-proxies/service-proxies';
...
@Component({
    selector: 'import-bmppartlist-modal',
    templateUrl: './import-bmppartlist-modal.component.html',
    styleUrls: ['./import-bmppartlist-modal.component.less']
  })
  export class ImportBmpPartListComponent extends AppComponentBase{
    ...
    constructor(
        ...
        private _partlist: BmpPartListComponent
      ) {
        super(injector);
        this.uploadUrl = AppConsts.remoteServiceBaseUrl + '/Prod/ImportMstBmpProcessFromExcel';
        {}
      }
      ngOnInit() {
    ...
    upload() {
        if(this.fileName != ''){
            ...
            .subscribe(response => {
                ...
                if (response.success ) {
                     this.mergeDataBmpPartList(response.result.mstPtsBmpPartList.result[0].guid);
                }
                else if (response.error != null || !response.result.mstPtsBmpPartList.result) {
    ...
    mergeDataBmpPartList(guid) {
        this.isLoading = true;
        this._service.mergeDataBmpPartList(guid)
            .pipe(
                finalize(() => {
                    this.isLoading = false;
                })
