...
import { InvCkdPartListServiceProxy, InvCkdProductionMappingServiceProxy } from '@shared/service-proxies/service-proxies';
...
@Component({
    ...
    selector: 'import-production-mapping',
    templateUrl: './import-production-mapping.component.html',
    styleUrls: ['../../../import-modal.less'],
})
export class ImportProductionMappingComponent extends AppComponentBase {
    @ViewChild('imgInput', { static: false }) InputVar: ElementRef;
    @ViewChild('ExcelFileUpload', { static: false }) excelFileUpload: FileUpload;
    @ViewChild('importModal', { static: true }) modal: ModalDirective;
    @ViewChild('errorListModal', { static: true }) errorListModal: ListErrorImportProductionMappingComponent;
    ...
    constructor(
        ...
        private _ckdprodMap: ProductionMappingComponent
    ) {
        super(injector);
        this.uploadUrl = AppConsts.remoteServiceBaseUrl + '/Prod/ImportProductionMappinFromExcel';
        { }
    }
    ngOnInit() {
    ...
    upload() {
        ...
        if (this.fileName != '') {
            ...
            this._httpClient
                ...
                .subscribe(response => {
                    if (response.success) {
                        if (response.result.prodMap.result.length == 0) {
                            this.exeptionDataImport();
                        }
                        else {
                            this.mergeCkdProductionMapping(response.result.prodMap.result[0].guid);
                        }
                    }
    ...
    mergeCkdProductionMapping(guid) {
        this.isLoading = true;
        this._service.mergeDataInvCkdProductionMapping(guid)
            .pipe(
                finalize(() => {
                    this.isLoading = false;
                })
