...
import { LgwLupLotUpPlanServiceProxy, MstLgaBp2ProcessServiceProxy, MstLgwUnpackingPartServiceProxy } from '@shared/service-proxies/service-proxies';
...
@Component({
  selector: 'import-bp2process-modal',
  templateUrl: './import-bp2process-modal.component.html',
  styleUrls: ['./import-bp2process-modal.component.less']
})
export class ImportBp2ProcessComponent   extends AppComponentBase {
    ...
    constructor(
      ...
      private _Bp2Process: Bp2ProcessComponent
    ) {
      super(injector);
      this.uploadUrl = AppConsts.remoteServiceBaseUrl + '/Prod/ImportMstLgaBp2ProcessFromExcel';
      {}
    }
    ngOnInit() {
    ...
    upload() {
            if(this.fileName != ''){
                ...
                .subscribe(response => {
                    if (response.success ) {
                            var duplicate = response.result.unpackingPart.result.filter(d=>d.exception != null)
                            if(duplicate[0].exception != null)
                            {
                                alert('Trùng lặp dòng số: ' + duplicate[0].exception);
                                this.close();
                            }
                            else
                            {
                                this.mergeMstLgaBp2Process(response.result.unpackingPart.result[0].guid);
                            }
    ...
    mergeMstLgaBp2Process(guid) {
        this.isLoading = true;
        this._service.mergeDataMstLgaBp2Process(guid)
            .pipe(
                finalize(() => {
                    this.isLoading = false;
                })
