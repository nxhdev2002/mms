...
import {  MstCmmDevanningCaseTypeServiceProxy, MstCmmDevanningCaseTypeDto } from '@shared/service-proxies/service-proxies';
...
export class DevanningCaseTypeComponent extends AppComponentBase implements OnInit {
    @ViewChild('viewHistoryDevanningCaseTypeModal', { static: true }) modal: ModalDirective | undefined
    ...
    changedRecordsDevanningCaseType: number[] = [];
    selectId;
    selectedRow: MstCmmDevanningCaseTypeDto = new MstCmmDevanningCaseTypeDto();
    saveSelectedRow: MstCmmDevanningCaseTypeDto = new MstCmmDevanningCaseTypeDto();
    datas: MstCmmDevanningCaseTypeDto = new MstCmmDevanningCaseTypeDto();
    ...
    constructor(
        injector: Injector,
        private _service: MstCmmDevanningCaseTypeServiceProxy,
    ...
    fetchChangedRecords() {
        this._service.getChangedRecords().subscribe((result) => {
            this.changedRecordsDevanningCaseType = result;
    ...
    onChangeRowSelection(params: { api: { getSelectedRows: () => MstCmmDevanningCaseTypeDto[] } }) {
        this.saveSelectedRow = params.api.getSelectedRows()[0] ?? new MstCmmDevanningCaseTypeDto();
    ...
    deleteRow(system: MstCmmDevanningCaseTypeDto): void {
    ...
    exportToExcel(): void {
        this._service.getDevanningCaseTypeToExcel(
			this.caseNo,
			this.carFamilyCode,
			this.shoptypeCode,
			this.supplierNo,
