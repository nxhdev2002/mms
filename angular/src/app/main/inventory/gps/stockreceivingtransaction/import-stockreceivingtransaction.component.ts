...
@Component({
  selector: 'import-stockreceivingtransaction',
  templateUrl: './import-stockreceivingtransaction.component.html',
    styleUrls: ['../../../import-modal.less'],
})
export class ImportGpsStockReceivingTransactionComponent extends AppComponentBase {
    @ViewChild('imgInput', { static: false }) InputVar: ElementRef;
    @ViewChild('ExcelFileUpload', { static: false }) excelFileUpload: FileUpload;
    @ViewChild('importModal', { static: true }) modal: ModalDirective;
   @ViewChild('errorListModal', { static: true }) errorListModal: ListErrorImportGpsStockReceivingTransactionComponent;
    ...
    constructor(
        ...
        private _stockIssuing: StockReceivingTransactionComponent
    ) {
        super(injector);
        this.uploadUrl = AppConsts.remoteServiceBaseUrl + '/Prod/ImportGpsStockReceivingTransactionFromExcel';
        { }
    }
    ngOnInit() {
    }
    ...
    upload() {
        ...
        if (this.fileName != '') {
            ...
            this._httpClient
                ...
                .subscribe(response => {
                    if (response.success) {
                        if (response.result.gpsStockReceiving.result.length == 0) {
                            this.exeptionDataImport();
                        }
                        else {
                            this.mergeDrmStockPart(response.result.gpsStockReceiving.result[0].guid);
                        }
                    }
                },
                (error) => {
    ...
    mergeDrmStockPart(guid) {
        this.isLoading = true;
        this._service.mergeDataInvGpsStockReceiving(guid)
            .pipe(
                finalize(() => {
